...
async function findAllCoursesForUser(userId) {
  let usersPaths = await db("users_paths as up")
    .select("up.path_id")
...
async function deleteCourseById(courseId) {
  ...
  if (!course) return { message: "No course found with that ID", code: 404 };
  let delReturn = await db("courses")
    .where({ id: courseId })
    .del();
  return { code: 200 };
=======
async function updateCourseById(user, courseId, changes) {
    ...
    if (!course) return { message: 'No course found with that ID', code: 404 }
    if (course.creator_id !== user.id && user.owner === false && user.admin === false && user.moderator === false) return { message: 'User is not permitted to change this course', code: 403 }
    await db('courses').where({ id: courseId }).update(changes)
    return { code: 200 }
}

async function deleteCourseById(user, courseId) {
    let courseObj = await findById(courseId)
    let course = courseObj.course
...
async function addCourseTag(user, courseId, tag) {
  ...
  if (tagId === -1) {
    ...
    await db("tags_courses").insert({ tag_id: tagId, course_id: courseId });
...
async function deleteCourseTag(user, courseId, tag) {
    ...
    if (!course) return { message: 'No course found with that ID', code: 404 }
    if (course.creator_id !== user.id && user.owner === false && user.admin === false && user.moderator === false) return { message: 'User is not permitted to remove tags from this course', code: 403 }
>>>>>>> 0bd7425cb4d966d3676dc8bf0199e8a001f1a660

  if (tagId === -1) {
    return { message: "Tag not found", code: 404 };
  } else
    await db("tags_courses")
      .where({ tag_id: tagId, course_id: courseId })
      .del();
...
async function findCourseSectionsByCourseId(id) {
  let details = await db("course_sections as cs").where({ "cs.course_id": id });
  return details;
}
...
async function addCourseSection(userId, courseId, section) {
  ...
  if (course.creator_id !== userId)
    return {
      message: "User is not permitted to add sections to this course",
      code: 403
    };
  section.course_id = courseId;
  let addReturn = await db("course_sections").insert(section, "id");
...
=======
async function addCourseSection(user, courseId, section) {
    let courseObj = await findById(courseId)
    let course = courseObj.course
    if (!course) return { message: 'No course found with that ID', code: 404 }
    if (course.creator_id !== user.id && user.owner === false && user.admin === false && user.moderator === false) return { message: 'User is not permitted to add sections to this course', code: 403 }
    section.course_id = courseId
...
async function updateCourseSection(user, courseId, sectionId, changes) {
    let courseObj = await findById(courseId)
    let course = courseObj.course
    if (!course) return { message: 'No course found with that ID', code: 404 }
    if (course.creator_id !== user.id && user.owner === false && user.admin === false && user.moderator === false) return { message: 'User is not permitted to update sections to this course', code: 403 }
    let updatereturn = await db('course_sections')
        .where({ id: sectionId })
        .update(changes)
    return { code: 200, message: updatereturn }
...
async function deleteCourseSection(user, courseId, sectionId) {
    let courseObj = await findById(courseId)
    let course = courseObj.course
    if (!course) return { message: 'No course found with that ID', code: 404 }
    if (course.creator_id !== user.id && user.owner === false && user.admin === false && user.moderator === false) return { message: 'User is not permitted to update sections to this course', code: 403 }
...
async function findSectionDetailsByCourseSectionsId(id) {
  let section = await db("section_details as sd").where({
    "sd.course_sections_id": id
  });
  return section;
}
...
async function addSectionDetails(userId, courseId, details) {
  ...
  // if (course.creator_id !== userId) return { message: 'User is not permitted to add Details to this Course Section', code: 403 }
    ...
    let addreturn = await db("section_details").insert(details, "id");
    await updateUsersLessonsOnLessonAdd(
      addreturn[0],
      details.course_sections_id
    );
...
async function deleteSectionDetails(courseId, sectionId, detailId) {
  ...
  return { code: 200, message: "delete successful" };
=======
async function addSectionDetails(user, courseId, details) {
    let courseObj = await findById(courseId)
    let course = courseObj.course
    if (!course) return { message: 'No course found with that ID', code: 404 }
    if (course.creator_id !== user.id && user.owner === false && user.admin === false && user.moderator === false) return { message: 'User is not permitted to add Details to this Course Section', code: 403 }
    else {
        let addreturn = await db('section_details')
            .insert(details, 'id')
...
async function updateSectionDetails(user, courseId, sectionId, detailId, changes) {
    let courseObj = await findById(courseId)
    let course = courseObj.course
...
async function deleteSectionDetails(user, courseId, sectionId, detailId) {
    let courseObj = await findById(courseId)
    let course = courseObj.course

    if (!course) return { message: 'No course found with that ID', code: 404 }
    if (course.creator_id !== user.id && user.owner === false && user.admin === false && user.moderator === false) return { code: 403, message: 'User is not permitted to add Details to this Course Section', code: 403 }
    await db('section_details')
        .where({ id: detailId, course_sections_id: sectionId })
        .del()
    return { code: 200, message: 'delete successful' }
...
async function lessonCascadeUp(userId, contentId) {
  try {
    let sectionId = await db("section_details as sd")
      ...
      .where({ id: contentId })
      .first();
    let userSectionDetails = await findUserSectionDetailsBySectionId(
      userId,
      sectionId.course_sections_id
    );
    isComplete = userSectionDetails.every(
      el => el.automatically_completed || el.manually_completed
    );
    userSectionDetails.forEach(el => {
      if (!el.manually_completed && !el.automatically_completed) {
        console.log(el);
      }
    });
    await db("users_sections as us")
      .where({
        "us.section_id": sectionId.course_sections_id,
        "us.user_id": userId
      })
...
async function sectionCascadeUp(userId, contentId) {
  try {
    let courseId = await db("course_sections as cs")
      ...
      .where({ id: contentId })
      .first();
    let userSections = await findUserSectionsByCourseId(
      userId,
      courseId.course_id
    );
    isComplete = userSections.every(
      el => el.automatically_completed || el.manually_completed
    );
...
async function courseCascadeUp(userId, contentId) {
  try {
    ...
    for (let i = 0; i < pathIds.length; i++) {
      let tempPathCourses = await findUserCoursesByPathId(
        userId,
        pathIds[i].path_id
      );
      let tempPathItems = await findYourPathItemsForPath(
        userId,
        pathIds[i].path_id
      );
      let isCoursesComplete = tempPathCourses.every(
        el => el.automatically_completed || el.manually_completed
      );
      let isItemsComplete = tempPathItems.every(
        el => el.automatically_completed || el.manually_completed
      );
...
async function itemCascadeUp(userId, contentId) {
  try {
    let pathId = await db("path_items as pi")
      .select("pi.path_id")
      .where({ id: contentId })
      .first();
    ...
    let isCoursesComplete = pathCourses.every(
      el => el.automatically_completed || el.manually_completed
    );
    let isItemsComplete = pathItems.every(
      el => el.automatically_completed || el.manually_completed
    );
    let isComplete = isCoursesComplete && isItemsComplete;
...
async function findUserSectionDetailsBySectionId(userId, sectionId) {
  try {
    return await db("course_sections as cs")
      ...
      .join(
        "users_section_details as usd",
        "usd.section_detail_id",
        "=",
        "sd.id"
      )
      .select("usd.*")
...
async function manualLessonCompleteToggle(
  userId,
  courseId,
  sectionId,
  sectionDetailId
  ...
  try {
    let userLesson = await db("users_section_details")
      .where({ user_id: userId, section_detail_id: sectionDetailId })
      .first();
    notIsCompleted = !(
      userLesson.manually_completed || userLesson.automatically_completed
    );
...
async function manualSectionCompleteToggle(userId, courseId, sectionId) {
  try {
    let userSection = await db("users_sections")
    ...
    for (let i = 0; i < usersSectionDetails.length; i++) {
      ...
      await db("users_section_details")
        .where({
          user_id: userId,
          section_detail_id: usersSectionDetails[i].section_detail_id
        })
...
async function manualCourseCompleteToggle(userId, courseId) {
  try {
    let userCourse = await db("users_courses")
      .where({ user_id: userId, course_id: courseId })
      .first();
    userCourse.manually_completed = !userCourse.manually_completed;
    if (!userCourse.manually_completed)
      userCourse.automatically_completed = userCourse.manually_completed;
    ...
    for (let i = 0; i < usersSections.length; i++) {
      ...
      await db("users_sections")
        .where({ user_id: userId, section_id: usersSections[i].section_id })
      ...
      for (let j = 0; j < usersSectionDetails.length; j++) {
        let lessonAutComp =
          usersSections[i].manually_completed || userCourse.manually_completed;
        // console.log('toggling lessons in section. Lesson ID:', usersSectionDetails[j].section_detail_id)
        await db("users_section_details")
          .where({
            user_id: userId,
            section_detail_id: usersSectionDetails[j].section_detail_id
          })
...
async function findYourCourseSectionsByCourseId(userId, courseId) {
  ...
  for (let i = 0; i < sectionIds.length; i++) {
    let tempSection = await db("course_sections as cs")
      .where({ "cs.id": sectionIds[i] })
      .first();

    let tempUserSection = userSections.find(
      el => el.section_id === sectionIds[i]
    );
    tempSection.manually_completed = tempUserSection.manually_completed;
    tempSection.automatically_completed =
      tempUserSection.automatically_completed;
...
async function findYoursById(userId, courseId) {
  ...
  let lessonArr = [];
  for (let i = 0; i < sectionIds.length; i++) {
    let sectionLessons = await findUserSectionDetailsBySectionId(
      userId,
      sectionIds[i]
    );
  ...
  let completed = lessonArr.filter(
    el => el.manually_completed || el.automatically_completed
  ).length;
...
async function autoCourseCompleteToggle(userId, courseId, isPathCompleted) {
  try {
    ...
    for (let i = 0; i < pathIds.length; i++) {
      ...
      let userCourses = await findUserCoursesByPathId(userId, pathIds[i].id);
      console.log("blah", userCourses);
      let isComplete = userCourses.every(
        el => el.automatically_completed || el.manually_completed
      );
      if (isComplete)
        await db("users_paths as up")
...
async function updateUsersLessonsOnLessonAdd(lessonId, sectionId) {
  ...
  for (let i = 0; i < sectionUsersIds.length; i++) {
    await db("users_section_details").insert({
      user_id: sectionUsersIds[i],
      section_detail_id: lessonId
    });
...
async function generateUdemyCourse(userId, link, results, details) {
  ...
  details.instructors.forEach((el, index) => {
    if (
      details.instructors.length > 1 &&
      index != details.instructors.length - 1
    ) {
  ...
  for (let i = 0; i < results.length; i++) {
    if (results[i]._class === "chapter") {
      ...
      let asset_type = "";
      if (results[i].asset && results[i].asset.asset_type)
        asset_type = results[i].asset.asset_type;
